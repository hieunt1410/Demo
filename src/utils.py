...
def mix_graph(raw_graph, threshold=10):
    ui_graph, bi_graph, ub_graph = raw_graph
    
    ii_graph = np.zeros((ui_graph.shape[1], ui_graph.shape[1]), dtype=np.int32)
    ...
    bb_graph = np.eye(ub_graph.shape[1]) + bb_graph
...
def split_hypergraph(H, device, split_num=16):
    ...
    H_split = [torch.sparse_coo_tensor(Hi).to(device) for Hi in H_list]