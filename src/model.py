...
class Demo(nn.Module):
    ...
    def init_fusion_weights(self):
        ...
        self.UB_layer_coefs = UB_layer_coefs.unsqueeze(-1).unsqueeze(-1).to(self.device)
        self.UI_layer_coefs = UI_layer_coefs.unsqueeze(-1).unsqueeze(-1).to(self.device)
        self.BI_layer_coefs = BI_layer_coefs.unsqueeze(-1).unsqueeze(-1).to(self.device)
